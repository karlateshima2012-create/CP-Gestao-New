...
export const EditorTab: React.FC<EditorTabProps> = ({ selectedContact, onSave, onCancel, onRefresh }) => {
  const initialData: Partial<Contact> = useMemo(() => ({
    name: '', phone: '', email: '', birthday: '', postalCode: '', province: '', city: '', address: '', instagram: '', notes: '',
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in pb-20 space-y-8 flex flex-col items-center">
      ...
      {selectedContact && (
        <Card className="p-6 md:p-8 w-full flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-10 border border-gray-200 dark:border-gray-800 shadow-xl bg-white dark:bg-gray-900 rounded-[24px]">
          {/* Top/Left Section: Avatar & Basic Info */}
          <div className="flex items-center gap-4 md:gap-6">
            <div className="w-16 h-16 md:w-20 md:h-20 shrink-0 rounded-full overflow-hidden flex items-center justify-center text-2xl md:text-3xl font-black bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500 shadow-inner">
              {formData.photo_url_full ? (
                <img src={formData.photo_url_full} alt={formData.name} className="w-full h-full object-cover rounded-full" />
              ) : (
            ...
            <div className="space-y-1.5 overflow-hidden">
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="text-xl md:text-3xl font-black tracking-tight text-slate-900 dark:text-white truncate">
                  {formData.name || 'Sem Nome'}
                </h2>
                <Badge color={(formData as any).loyaltyLevel === 4 ? 'diamond' : (formData as any).loyaltyLevel === 3 ? 'gold' : (formData as any).loyaltyLevel === 2 ? 'silver' : 'bronze'} className="scale-90 md:scale-100 origin-left">
              ...
              <p className="text-sm md:text-lg font-bold text-slate-500 dark:text-slate-400">{formData.phone}</p>
            </div>
          </div>
          ...
          {/* Bottom/Right Section: Balance & Action Button */}
          <div className="flex items-center justify-between md:flex-col md:justify-center gap-6 md:gap-2 bg-slate-50/50 dark:bg-slate-800/30 md:bg-transparent p-4 md:p-0 rounded-2xl md:rounded-none">
            <div className="text-left md:text-center">
              <p className="text-[9px] md:text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-0.5">Saldo de Pontos</p>
              <div className="flex items-baseline md:items-center md:justify-center gap-1">
                <span className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tabular-nums">{formData.pointsBalance || 0}</span>
            ...
            <Button
              ...
              className="h-11 md:h-9 px-6 text-[10px] font-black uppercase tracking-widest rounded-xl md:rounded-[12px] shadow-lg shadow-blue-500/20 bg-[#38B6FF] hover:bg-[#38B6FF]/90 text-white border-none transition-all active:scale-95"
            >
              + PONTO
            </Button>
          </div>
      ...
      <Card className="p-8 border border-gray-200 dark:border-gray-800 w-full rounded-[24px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Input label={<>E-MAIL <span className="text-gray-400 dark:text-gray-600 font-medium ml-1">(OPCIONAL)</span></>} type="email" value={formData.email || ''} onChange={e => setFormData(p => ({ ...p, email: e.target.value }))} />
          <Input label={<>NOME DA EMPRESA <span className="text-gray-400 dark:text-gray-600 font-medium ml-1">(OPCIONAL)</span></>} value={formData.company_name || ''} onChange={e => setFormData(p => ({ ...p, company_name: e.target.value }))} />
          <Input label={<>CÓDIGO POSTAL <span className="text-gray-400 dark:text-gray-600 font-medium ml-1">(OPCIONAL)</span></>} value={formData.postalCode || ''} onChange={e => setFormData(p => ({ ...p, postalCode: e.target.value }))} />
          ...
          <div className="lg:col-span-2"><Input label={<>ENDEREÇO <span className="text-gray-400 dark:text-gray-600 font-medium ml-1">(OPCIONAL)</span></>} value={formData.address || ''} onChange={e => handleCapitalize('address', e.target.value)} /></div>
          <Input label={<>@ INSTAGRAM <span className="text-gray-400 dark:text-gray-600 font-medium ml-1">(OPCIONAL)</span></>} value={formData.instagram || ''} onChange={e => setFormData(p => ({ ...p, instagram: e.target.value }))} />
        </div>
      </Card>
        ...
        <div className="grid grid-cols-1 gap-6">
          <div className="p-6 bg-gray-50 dark:bg-gray-800/50 rounded-[20px] border border-gray-100 flex flex-col space-y-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">ORIGEM DO CLIENTE <span className="text-gray-400 dark:text-gray-600 font-medium ml-1">(OPCIONAL)</span></label>
            ...
            <div className="flex flex-col flex-1">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2 ml-1">Notas <span className="text-gray-400 dark:text-gray-600 font-medium ml-1">(OPCIONAL)</span></label>
          ...
          <h3 className="text-sm font-black uppercase tracking-widest flex items-center justify-between mb-8">
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="text-center">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Total Gasto</p>
                <div className="flex items-center justify-center gap-1">
                  <span className="text-2xl font-black text-gray-900 dark:text-white">¥ {Number(formData.totalSpent || 0).toLocaleString('en-US')}</span>
                </div>
              </div>
              <Button
                variant="primary"
                onClick={() => setServiceModal(true)}
                className="h-9 px-6 text-[10px] font-black uppercase tracking-widest rounded-[12px] shadow-lg shadow-blue-500/20"
              >
                + NOVO
              </Button>
            </div>
      ...
      <div className="w-full pt-8 pb-12 border-t border-gray-100 dark:border-gray-800">
        <Button
          ...
          className="w-full h-14 bg-[#38B6FF] hover:bg-[#38B6FF]/90 text-white shadow-xl shadow-blue-500/10 font-black text-xs uppercase tracking-[0.2em] rounded-2xl transition-all border-none"
        >
      ...
      {(actionModal || showConfirmLarge) && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <Card className="w-full max-w-sm p-8 space-y-6 shadow-2xl border-none animate-in zoom-in-95 duration-200">
            {showConfirmLarge ? (
              <>
                <div className="w-20 h-20 bg-amber-50 dark:bg-amber-900/30 rounded-lg flex items-center justify-center mx-auto text-amber-500">
