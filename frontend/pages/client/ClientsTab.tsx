    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-1">
        <div className="hidden md:block">
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight leading-none">Meus Clientes</h1>
          <p className="text-lg text-slate-500 dark:text-slate-400 mt-1 font-medium">Gerencie seu portfólio de {contacts.length} clientes.</p>
        </div>
        <div className="flex gap-3">
          <Button 
            onClick={onNew} 
            className="h-14 px-10 rounded-2xl text-xs font-black uppercase tracking-[0.2em] bg-[#38B6FF] text-white shadow-xl shadow-blue-500/10 hover:opacity-90 transition-all border-none"
          >
            + Novo Cliente
          </Button>
        </div>
                ...
                >
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      <div className="w-11 h-11 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-800 flex items-center justify-center font-black text-gray-700 dark:text-gray-300 shadow-inner group-hover:bg-white dark:group-hover:bg-gray-700 transition-colors">
                        {contact.photo_url_full ? (
                          <img src={contact.photo_url_full} alt={contact.name} className="w-full h-full object-cover rounded-full" />
                        ) : (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {(() => {
                            ...
                            if (createdAt) {
                              ...
                              if (isNew) {
                                return (
                                  <div className="flex items-center gap-1 bg-primary-500 text-white text-[9px] font-black px-2 py-0.5 rounded-lg shadow-sm shadow-primary-500/30 animate-pulse transition-all">
                  ...
                  <td className="px-8 py-5 text-right">
                    <div className="flex items-center justify-end gap-3 transition-opacity">
                      <Button
                        ...
                        className="h-14 px-6 bg-[#38B6FF] text-white rounded-2xl text-xs font-black uppercase tracking-[0.2em] shadow-xl shadow-blue-500/10 hover:opacity-90 transition-all border-none"
                      >
                        EDITAR
                      </Button>
                      <button
                        onClick={(e) => { e.stopPropagation(); onDelete(contact.id); }}
                        className="p-2 text-gray-400 hover:text-red-500 hover:bg-white dark:hover:bg-gray-800 rounded-xl transition-all shadow-sm hover:shadow-md border border-transparent hover:border-gray-100 dark:hover:border-gray-700"
                        title="Excluir"
                      >
