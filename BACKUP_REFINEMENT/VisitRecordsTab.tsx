...
export const VisitRecordsTab: React.FC = () => {
    ...
    useEffect(() => {
        // If there are pending points, default the status filter to 'pendente' on first load
        if (pendingCount > 0 && status === 'all') {
            setStatus('pendente');
        }
    }, [pendingCount]);
            ...
            <div className="flex flex-col gap-8">
                <div>
                    <h1 className="text-4xl font-black text-gray-900 dark:text-white tracking-tight flex items-center gap-4">
                        Aprovação de Pontos
                        {pendingCount > 0 && (
                            <span className="bg-primary-500 text-white text-[10px] font-black px-3 py-1.5 rounded-full animate-pulse shadow-lg shadow-primary-500/20">
                                {pendingCount} PENDENTES
                            </span>
                        )}
                    </h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-1 font-medium text-lg">Gerencie as visitas e valide os benefícios dos seus clientes.</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex bg-white dark:bg-gray-900 p-1.5 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm">
                        {['all', 'today', '7days', '30days'].map((p) => (
                            ...
                            >
                                {p === 'all' ? 'Sempre' : p === 'today' ? 'Hoje' : p === '7days' ? '1 Semana' : '1 Mês'}
                            </button>
                        ))}
                    </div>
                    ...
                    <select
                        ...
                        className="h-12 px-6 rounded-2xl bg-white dark:bg-gray-900 border-2 border-gray-100 dark:border-gray-800 text-[11px] font-black uppercase tracking-widest text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-primary-500 transition-all outline-none shadow-sm"
                    >
                    ...
                    {pendingCount > 0 && (
                        <Button
                            ...
                            className="h-12 px-10 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white text-[11px] font-black uppercase tracking-widest shadow-xl shadow-emerald-500/20 transition-all hover:scale-105 active:scale-95"
                        >
                            <CheckCircle2 className="w-4 h-4 mr-2" /> Aprovar Tudo
                        </Button>
                    )}
                </div>
            </div>
                                ...
                                visits.map((v) => (
                                    <tr key={v.id} className="group hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                                        ...
                                        <td className="px-4 py-5 text-center">
                                            <span className={`text-sm font-black ${v.points_granted > 0 ? 'text-emerald-500' : 'text-primary-500'}`}>
