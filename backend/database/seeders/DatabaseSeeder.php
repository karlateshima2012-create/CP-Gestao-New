...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Criar terminal padrão para Sushi Elite ver o PIN no Dashboard
        \App\Models\Device::updateOrCreate(
            ['tenant_id' => $eliteTenant->id, 'name' => 'Terminal Principal'],
            [
                'mode' => 'nfc_qr',
                'active' => true,
                'require_pin' => true,
                'current_pin_code' => '888',
                'pin_generated_at' => now(),
            ]
        );

        $this->command->info('⏳ Gerando base de 5.000 clientes para Sushi Elite (Ilimitado)...');
        $this->generateCustomers($eliteTenant->id, 5000);
