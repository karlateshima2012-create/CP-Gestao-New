...
class ClientController extends Controller
{
    ...
    public function toggleDeviceStatus(Request $request, $deviceId)
    {
        $device = \App\Models\Device::where('tenant_id', $request->user()->tenant_id)->findOrFail($deviceId);
    ...
    public function getDashboardMetrics(Request $request)
    {
        try {
            ...
            $device = \App\Models\Device::where('tenant_id', $tenantId)->first();
            $firstDevice = null;
            if ($device) {
                 $firstDevice = [
                     'id' => $device->id,
            ...
            return ApiResponse::ok([
                ...
                'dashboard_sound_enabled' => $tenant->settings->dashboard_sound_enabled ?? true,
            ]);
        } catch (\Throwable $e) {
